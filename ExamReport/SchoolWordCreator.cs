...
namespace ExamReport
{
    public class SchoolWordCreator
    {
        ...
        public void creating_word_part2()
        {
            ...
            insertText(ExamTitle1, "总体分析");
            if(Utils.isNewGK)
                insertTotalTable_sample("    总分分析表", _pdata);
            else
            ...
            if(Utils.isNewGK)
                insertTotalTopic_sample("    题目整体分析表", _pdata);
            else
                insertTotalTopic("    题目整体分析表", _pdata);
            if(Utils.isNewGK)
                insertGroupTopic_sample("    题组整体分析表", _pdata, ((PartitionData)_pdata[2]).groups_analysis.Rows.Count);
            else
            ...
            if(Utils.isNewGK)
                insertTotalTupleTable("    总体分组分析表", _pdata);
            oDoc.Characters.Last.InsertBreak(oPagebreak);
            insertText(ExamTitle1, "题组分析");
            ...
            for (int i = 0; i < _groups_group.Count; i++)
            {
                ...
                foreach (string group in _groups_group[keys[i]])
                {
                    ...
                    if(Utils.isNewGK)
                        insertSingleGroupTotal_sample("    " + group + "总分分析表", group_count, true, _pdata);
                    else
                    ...
                    if (Utils.isNewGK)
                    {
                        insertGroupDiffChart("    " + group + "得分率曲线图", ((WordData.group_data)_sdata.single_group_analysis[group_count]).group_difficulty);

                        int columnnum = ((WordData.group_data)_sdata.single_group_analysis[group_count]).group_detail.Columns.Count;
                        int rownum = ((WordData.group_data)_sdata.single_group_analysis[group_count]).group_detail.Rows.Count;
                        ((WordData.group_data)_sdata.single_group_analysis[group_count]).group_detail.Columns.RemoveAt(columnnum - 3);
                        ((WordData.group_data)_sdata.single_group_analysis[group_count]).group_detail.Rows.RemoveAt(rownum - 2);

                        insertGroupSingleAnalysis_sample("    " + group + "分组分析表", ((WordData.group_data)_sdata.single_group_analysis[group_count]).group_detail);
                    }
                    else
                    {
            ...
            for (int i = 0; i < ((PartitionData)_pdata[2]).total_analysis.Rows.Count; i++)
            {
                ...
                if (Utils.isNewGK)
                {
                    insertSingleGroupTotal_sample("    " + numstr + "分析表", i, false, _pdata);
                    insertSingleChart("    " + numstr + "得分率曲线图", ((WordData.single_data)_sdata.single_topic_analysis[i]).single_difficulty, "分数", "得分率", Excel.XlChartType.xlXYScatterSmooth);
                    insertMultipleChart("    " + numstr + "分组得分率曲线图", ((WordData.single_data)_sdata.single_topic_analysis[i]).single_dist, "分组", "得分率", Excel.XlChartType.xlLineMarkers);

                    int columnum = ((WordData.single_data)_sdata.single_topic_analysis[i]).single_detail.Columns.Count;
                    int rownum = ((WordData.single_data)_sdata.single_topic_analysis[i]).single_detail.Rows.Count;
                    ((WordData.single_data)_sdata.single_topic_analysis[i]).single_detail.Columns.RemoveAt(columnum - 3);
                    ((WordData.single_data)_sdata.single_topic_analysis[i]).single_detail.Rows.RemoveAt(rownum - 2);

                    insertGroupTable_sample("    " + numstr + "分组分析表", ((WordData.single_data)_sdata.single_topic_analysis[i]).single_detail, ((WordData.single_data)_sdata.single_topic_analysis[i]).stype);
                }
                else
                {
        ...
        public void insertTotalTupleTable(string title, List<WSLG_partitiondata> totaldata)
        {
            ...
            table = oDoc.Tables.Add(range, totaldata.Count * 2 + 1, groupnum + 2, ref oMissing, oTrue);
            ...
            for (int i = 1; i < 3; i++)
            {
                int tablerow = (i-1) * totaldata.Count + 2;
        ...
        public void insertGroupTable_sample(string title, DataTable dt, WordData.single_type type)
        {
        ...
        public void insertGroupSingleAnalysis_sample(string title, DataTable single_group_dt)
        {
        ...
        public void insertSingleGroupTotal_sample(string title, int total, bool isGroup, List<WSLG_partitiondata> sdata)
        {
            Word.Table table;
            Word.Range range = oDoc.Bookmarks.get_Item(ref oEndOfDoc).Range;

            table = oDoc.Tables.Add(range, sdata.Count + 1, 9, ref oMissing, oTrue);
            table.Range.InsertCaption(oWord.CaptionLabels["表"], title, oMissing, Word.WdCaptionPosition.wdCaptionPositionAbove, oMissing);
            range.MoveEnd(Word.WdUnits.wdParagraph, 1);
            range.Paragraphs.Alignment = Word.WdParagraphAlignment.wdAlignParagraphCenter;
            table.Rows[1].HeadingFormat = -1;
            ...
            table.Cell(1, 5).Range.Text = "平均值";
            table.Cell(1, 6).Range.Text = "标准差";
            table.Cell(1, 7).Range.Text = "差异系数";
            table.Cell(1, 8).Range.Text = "得分率";
            table.Cell(1, 9).Range.Text = "鉴别指数";
            ...
            for (int i = 0; i < sdata.Count; i++)
            {
                ...
                table.Cell(i + 2, 2).Range.Text = FullmarkFormat((decimal)data.Rows[total]["fullmark"]);
                table.Cell(i + 2, 3).Range.Text = string.Format("{0:F1}", data.Rows[total]["max"]);
                table.Cell(i + 2, 4).Range.Text = string.Format("{0:F1}", data.Rows[total]["min"]);
                table.Cell(i + 2, 5).Range.Text = string.Format("{0:F2}", data.Rows[total]["avg"]);
                table.Cell(i + 2, 6).Range.Text = string.Format("{0:F2}", data.Rows[total]["stDev"]);
                table.Cell(i + 2, 7).Range.Text = string.Format("{0:F2}", data.Rows[total]["dfactor"]);
                table.Cell(i + 2, 8).Range.Text = string.Format("{0:F2}", data.Rows[total]["difficulty"]);

                if (isGroup)
                    table.Cell(i + 2, 9).Range.Text = string.Format("{0:F2}", ((WSLG_partitiondata)partition).group_discriminant[total]);
                else
                    table.Cell(i + 2, 9).Range.Text = string.Format("{0:F2}", ((WSLG_partitiondata)partition).total_discriminant[total]);
        ...
        public void insertGroupTopic_sample(string title, List<WSLG_partitiondata> totaldata, int count)
        {
            //int count = ((PartitionData)totaldata[totaldata.Count - 1]).groups_analysis.Rows.Count;
            Word.Table table;
            Word.Range range = oDoc.Bookmarks.get_Item(ref oEndOfDoc).Range;

            table = oDoc.Tables.Add(range, totaldata.Count * count + 1, 10, ref oMissing, oTrue);
            table.Range.InsertCaption(oWord.CaptionLabels["表"], title, oMissing, Word.WdCaptionPosition.wdCaptionPositionAbove, oMissing);
            range.MoveEnd(Word.WdUnits.wdParagraph, 1);
            range.Paragraphs.Alignment = Word.WdParagraphAlignment.wdAlignParagraphCenter;
            table.Rows[1].HeadingFormat = -1;
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                foreach (PartitionData data in totaldata)
                {
                    ...
                    table.Cell(tablerow, 3).Range.Text = FullmarkFormat((decimal)data.groups_analysis.Rows[i]["fullmark"]);
                    table.Cell(tablerow, 4).Range.Text = string.Format("{0:F1}", data.groups_analysis.Rows[i]["max"]);
                    table.Cell(tablerow, 5).Range.Text = string.Format("{0:F1}", data.groups_analysis.Rows[i]["min"]);
                    table.Cell(tablerow, 6).Range.Text = string.Format("{0:F2}", data.groups_analysis.Rows[i]["avg"]);
                    table.Cell(tablerow, 7).Range.Text = string.Format("{0:F2}", data.groups_analysis.Rows[i]["stDev"]);
                    table.Cell(tablerow, 8).Range.Text = string.Format("{0:F2}", data.groups_analysis.Rows[i]["dfactor"]);
                    table.Cell(tablerow, 9).Range.Text = string.Format("{0:F2}", data.groups_analysis.Rows[i]["difficulty"]);
                    table.Cell(tablerow, 10).Range.Text = string.Format("{0:F2}", ((WSLG_partitiondata)data).group_discriminant[i]);
                    tablerow++;
                }
            }
            table.Select();
        ...
        public void insertTotalTopic_sample(string title, List<WSLG_partitiondata> totaldata)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                foreach (PartitionData data in totaldata)
                {
                    ...
                    table.Cell(tablerow, 3).Range.Text = FullmarkFormat((decimal)data.total_analysis.Rows[i]["fullmark"]);
                    table.Cell(tablerow, 4).Range.Text = string.Format("{0:F1}", data.total_analysis.Rows[i]["max"]);
                    table.Cell(tablerow, 5).Range.Text = string.Format("{0:F1}", data.total_analysis.Rows[i]["min"]);
                    table.Cell(tablerow, 6).Range.Text = string.Format("{0:F2}", data.total_analysis.Rows[i]["avg"]);
                    table.Cell(tablerow, 7).Range.Text = string.Format("{0:F2}", data.total_analysis.Rows[i]["stDev"]);
                    table.Cell(tablerow, 8).Range.Text = string.Format("{0:F2}", data.total_analysis.Rows[i]["dfactor"]);
                    table.Cell(tablerow, 9).Range.Text = string.Format("{0:F2}", data.total_analysis.Rows[i]["difficulty"]);
                    table.Cell(tablerow, 10).Range.Text = string.Format("{0:F2}", ((WSLG_partitiondata)data).total_discriminant[i]);
                    tablerow++;
                }
            }
            table.Select();
        ...
        public void insertTotalTable_sample(string title, List<WSLG_partitiondata> totaldata)
        {
            Word.Table table;
            Word.Range range = oDoc.Bookmarks.get_Item(ref oEndOfDoc).Range;
            ...
            for (int i = 0; i < totaldata.Count; i++)
            {
                ...
                table.Cell(i + 2, 2).Range.Text = FullmarkFormat((decimal)((PartitionData)totaldata[i]).fullmark);
                table.Cell(i + 2, 3).Range.Text = string.Format("{0:F1}", ((PartitionData)totaldata[i]).max);
                table.Cell(i + 2, 4).Range.Text = string.Format("{0:F1}", ((PartitionData)totaldata[i]).min);
                table.Cell(i + 2, 5).Range.Text = string.Format("{0:F2}", ((PartitionData)totaldata[i]).avg);
                table.Cell(i + 2, 6).Range.Text = string.Format("{0:F2}", ((PartitionData)totaldata[i]).stDev);
                table.Cell(i + 2, 7).Range.Text = string.Format("{0:F2}", ((PartitionData)totaldata[i]).Dfactor);
                table.Cell(i + 2, 8).Range.Text = string.Format("{0:F2}", ((PartitionData)totaldata[i]).difficulty);
                table.Cell(i + 2, 9).Range.Text = string.Format("{0:F2}", ((WSLG_partitiondata)totaldata[i]).discriminant);
            }
