...
namespace ExamReport
{
    public class Partition_wordcreator
    {
        ...
        public void creating_word_part2()
        {
            ...
            insertText(ExamTitle1, "总体分析");
            if (Utils.isNewGK && _report_type.Equals("区县"))
            {
                insertTotalTable_Sample("    总分分析表", _totaldata);
            }
            else {
            ...
            if (Utils.isNewGK && _report_type.Equals("区县"))
            {
                insertTotalTopic_sample("    题目整体分析表", _totaldata);
                insertGroupTopic_sample("    题组整体分析表", _totaldata, ((PartitionData)_totaldata[_totaldata.Count - 1]).groups_analysis.Rows.Count);
            }
            else
            {
                if (_report_type.Equals("区县"))
            ...
            if(!Utils.isNewGK)
                insertFreqTable("    总分频数分布表", _sdata);
            if(Utils.isNewGK)
                insertTotalTupleTable_sample("    总体分组分析表", _sdata);
            else
                insertTotalTupleTable("    总体分组分析表", _sdata);
            oDoc.Characters.Last.InsertBreak(oPagebreak);
            insertText(ExamTitle1, "题组分析");
            ...
            for (int i = 0; i < _groups_group.Count; i++)
            {
                ...
                foreach (string group in _groups_group[keys[i]])
                {
                    ...
                    insertTH(_groups.Rows[group_count]["th"].ToString().Trim());
                    if(Utils.isNewGK)
                        insertSingleGroupTotal_sample("    " + group + "总分分析表", group_count, true, _sdata);
                    else
                        insertSingleGroupTotal("    " + group + "总分分析表", group_count, true, _sdata);
                    ChartCombine tempdata = new ChartCombine();
                    for (int j = 0; j < _sdata.Count; j++)
                    ...
                    insertChart("    " + group + "分数分布图", tempdata.target, "分数", "比率(%)", Excel.XlChartType.xlLineMarkers, (decimal)((PartitionData)_sdata[0]).groups_analysis.Rows[group_count]["fullmark"]);
                    if(Utils.isNewGK)
                        insertSingleGrouptuple_sample("    " + group + "分组分析表", group_count, _sdata);
                    else
                        insertSingleGrouptuple("    " + group + "分组分析表", group_count, _sdata);
                    oDoc.Characters.Last.InsertBreak(oPagebreak);
                    group_count++;
            ...
            for (int i = 0; i < ((PartitionData)_sdata[_sdata.Count - 1]).total_analysis.Rows.Count; i++)
            {
                ...
                insertText(ExamTitle3, numstr);
                if(Utils.isNewGK)
                    insertSingleGroupTotal_sample("    " + numstr + "分析表", i, false, _sdata);
                else
                    insertSingleGroupTotal("    " + numstr + "分析表", i, false, _sdata);
                if(Utils.isNewGK)
                    insertSingleTopictuple_sample("    " + numstr + "分组分析表", i, getSingleData(_sdata.Count - 1, i).stype);
                else
                    insertSingleTopictuple("    " + numstr + "分组分析表", i, getSingleData(_sdata.Count - 1, i).stype);
                oDoc.Characters.Last.InsertBreak(oPagebreak);
            }
        ...
        public void insertSingleTopictuple_sample(string title, int topicnum, WordData.single_type type)
        {
            for(int i = 0; i < _sdata.Count; i++)
            {
                DataTable dtable = getSingleData(i, topicnum).single_detail;
                dtable.Columns.Remove(dtable.Columns["frequency"]);
                dtable.Rows.Remove(dtable.Rows.Find("合计"));
            }
            Word.Table table;
                    ...
                    else
                    {
                        ...
                        else if (dr["mark"].ToString().Trim().Equals("得分率"))
                        {
                            ...
                            for (j = 1; j < colnum - 2; j++)
                                table.Cell(temp_rownum, j + 2).Range.Text = "0.00";
                            //table.Cell(temp_rownum, j + 2).Range.Text = "0.00";
                            table.Cell(temp_rownum, j + 2).Range.Text = "-";
                            table.Cell(temp_rownum, j + 3).Range.Text = "-";
        ...
        public void insertSingleGrouptuple_sample(string title, int group, ArrayList sdata)
        {
            for(int i = 0; i < sdata.Count; i++)
            {
                DataTable dt = getGroupData(sdata, i, group).group_detail;
                dt.Columns.Remove(dt.Columns["frequency"]);
                dt.Rows.Remove(dt.Rows.Find("合计"));
            }
            ...
            table.Cell(1, 1).Range.Text = "分值";
            table.Cell(1, 2).Range.Text = "分类";
            for (colnum = 1; colnum < col - 2; colnum++)
            ...
            table.Cell(1, colnum + 2).Range.Text = "比率(%)";
            table.Cell(1, colnum + 3).Range.Text = "平均值";
        ...
        public void insertSingleGroupTotal_sample(string title, int total, bool isGroup, ArrayList sdata)
        {
            Word.Table table;
            Word.Range range = oDoc.Bookmarks.get_Item(ref oEndOfDoc).Range;
            int col = 8;
            ...
            for (int i = 0; i < sdata.Count; i++)
            {
                ...
                else
                {
                    ...
                    if (total >= data.Rows.Count)
                    {
                        ...
                        table.Cell(i + 2, 2).Range.Text = FullmarkFormat((decimal)((PartitionData)sdata[sdata.Count - 1]).total_analysis.Rows[total]["fullmark"]);
                        table.Cell(i + 2, 3).Range.Text = "0.0";
                        table.Cell(i + 2, 4).Range.Text = "0.0";
                        table.Cell(i + 2, 5).Range.Text = "0.00";
        ...
        public void insertTotalTupleTable_sample(string title, ArrayList totaldata)
        {
            int groupnum = ((PartitionData)totaldata[0]).Total_tuple_analysis.Rows.Count;
            Word.Table table;
            Word.Range range = oDoc.Bookmarks.get_Item(ref oEndOfDoc).Range;

            table = oDoc.Tables.Add(range, 4, groupnum + 2, ref oMissing, oTrue);
            table.Range.InsertCaption(oWord.CaptionLabels["表"], title, oMissing, Word.WdCaptionPosition.wdCaptionPositionAbove, oMissing);
            range.MoveEnd(Word.WdUnits.wdParagraph, 1);
            range.Paragraphs.Alignment = Word.WdParagraphAlignment.wdAlignParagraphCenter;
            table.Rows[1].HeadingFormat = -1;
            ...
            {
                int tablerow = i + 2;
                int rcount = totaldata.Count;
                table.Cell(tablerow, 1).Range.Text = name[i];
                table.Cell(tablerow, 2).Range.Text = ((PartitionData)totaldata[rcount - 1]).name;
                for (int k = 0; k < groupnum; k++)
                    table.Cell(tablerow, k + 3).Range.Text = ((PartitionData)totaldata[rcount-1]).Total_tuple_analysis.Rows[k][i + 1].ToString().Trim();
        ...
        public void insertGroupTopic_sample(string title, ArrayList totaldata, int count)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                foreach (PartitionData data in totaldata)
                {
                    ...
                    table.Cell(tablerow, 10).Range.Text = string.Format("{0:F2}", data.group_discriminant[i]);
                    tablerow++;
                }
            }
            table.Select();
        ...
        public void insertTotalTopic_sample(string title, ArrayList totaldata)
        {
            int pcount = 0;
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                foreach (PartitionData data in totaldata)
                {
                    ...
                    table.Cell(tablerow, 10).Range.Text = string.Format("{0:F2}", data.total_discriminant[i]);
                    tablerow++;
                }
            }
        ...
        public void insertTotalTable_Sample(string title, ArrayList totaldata)
        {
            Word.Table table;
            Word.Range range = oDoc.Bookmarks.get_Item(ref oEndOfDoc).Range;
            int count = 9;
            ...
            for (int i = 0; i < totaldata.Count; i++)
            {
                ...
                table.Cell(i + 2, 9).Range.Text = string.Format("{0:F2}", ((PartitionData)totaldata[i]).discriminant);
            }
