...
//                              temp_dr["mark"] = string.Format("{0:F1}",  item.mark) + "～";
        ...
        public bool statistic_sample_process(bool isZonghe)
        {
            result.total_num = _basic_data.Rows.Count;
            result.fullmark = _fullmark;
            string totalmark_str;
            ...
            if (result.fullmark > 20.0m)
            {
                ...
                for (j = 0; j < 20; j++)
                {
                    DataRow inter_row = result.totalmark_dist_sample.NewRow();
                    inter_row["mark"] = flag;
                    inter_row["num"] = 0;
                    flag += total_interval;
                    result.totalmark_dist_sample.Rows.Add(inter_row);
                }
                if ((result.fullmark - 20.0m * total_interval) != 0)
                {
                    DataRow last_row = result.totalmark_dist_sample.NewRow();
                    last_row["mark"] = 20.0m * total_interval + (result.fullmark - 20.0m * total_interval + 1) / 2.0m;
                    last_row["num"] = 0;
                    result.totalmark_dist_sample.Rows.Add(last_row);
                }
            }
            else
            {
            ...
            foreach (var item in freq)
            {
                ...
                if (total_interval == 1.0m)
                    ...
                    dist_num = Convert.ToInt32(Math.Ceiling((decimal)dr["totalmark"] / total_interval));
                if (dist_num > 20)
                    result.totalmark_dist_sample.Rows[20]["num"] = (decimal)result.totalmark_dist_sample.Rows[20]["num"] + (int)dr["frequency"];
                else if (dist_num == 0)
                    result.totalmark_dist_sample.Rows[dist_num]["num"] = (decimal)result.totalmark_dist_sample.Rows[dist_num]["num"] + (int)dr["frequency"];
                else
                    result.totalmark_dist_sample.Rows[dist_num - 1]["num"] = (decimal)result.totalmark_dist_sample.Rows[dist_num - 1]["num"] + (int)dr["frequency"];
            ...
            foreach (DataRow dr in result.totalmark_dist_sample.Rows)
            {
                dr["num"] = (decimal)dr["num"] / Convert.ToDecimal(result.total_num) * 100;
            }
            ...
            foreach (DataRow dr in result.group_analysis.Rows)
            {
                ...
                temp.group_dist.Columns.Add("mark", typeof(decimal));
                temp.group_dist.Columns.Add("num", typeof(decimal));
                ...
                foreach (var item in single_group)
                {
                    ...
                    //if (dist_num == 0)
                    temp.group_dist.Rows[dist_num]["num"] = (decimal)temp.group_dist.Rows[dist_num]["num"] + item.count;
                    //else
                    //    temp.group_dist.Rows[dist_num - 1]["num"] = (int)temp.group_dist.Rows[dist_num - 1]["num"] + item.count;
                ...
                foreach (DataRow drow in temp.group_dist.Rows)
                {
                    drow["num"] = (decimal)drow["num"] / Convert.ToDecimal(result.total_num) * 100;
                }
                foreach (DataRow row in temp.group_detail.Rows)
                {
                    row["rate"] = ((int)row["frequency"] / Convert.ToDecimal(result.total_num)) * 100;
                ...
                foreach (DataRow item in temp.group_detail.Rows)
                {
                    if((int)item["frequency"] != 0)
                        for (i = 1; i <= _groupnum; i++)
                            item["G" + i.ToString()] = (decimal)item["G" + i.ToString()] / (int)item["frequency"] * 100;
                }
            ...
            if (!isZonghe)
            {
                ...
                foreach (DataRow dr in result.total_analysis.Rows)
                {
                    ...
                    if (!_standard_ans.Rows.Find(dr["number"].ToString().Substring(1))["da"].Equals(""))
                    {
                        ...
                        {
                            DataRow groups_row = temp.single_detail.Rows.Find(choiceTransfer(item.choice.ToString()));
                            groups_row[item.groups.ToString().Trim()] = (decimal)groups_row[item.groups.ToString().Trim()] + Convert.ToDecimal(item.count);
                        }

                        foreach (DataRow drow in temp.single_detail.Rows)
                        {
                            if((int)drow["frequency"] != 0)
                                for (i = 1; i <= _groupnum; i++)
                                    drow["G" + i.ToString()] = (decimal)drow["G" + i.ToString()] / (int)drow["frequency"] * 100;
                        }
                        ...
                        single_avg_row["frequency"] = 0;
                        if(Utils.isNewGK)
                            single_avg_row["rate"] = dr["difficulty"];
                        else
                            single_avg_row["rate"] = 0m;
                        single_avg_row["correlation"] = 0m;
                        ...
                        if (_standard_ans.Rows.Find(dr["number"].ToString().Substring(1))["da"].ToString().Trim().Length == 1)
                        {
                            ...
                            foreach (DataRow temp_dr in temp.single_detail.Rows)
                            {
                                ...
                                nochoice_row["frequency"] = (int)nochoice_row["frequency"] + (int)temp_dr["frequency"];
                                for (i = 1; i <= _groupnum; i++)
                                    nochoice_row["G" + i.ToString().Trim()] = (decimal)nochoice_row["G" + i.ToString().Trim()] + ((decimal)temp_dr["G" + i.ToString().Trim()] * (int)temp_dr["frequency"])/100;

                            }
                            if ((int)nochoice_row["frequency"] != 0)
                                for (i = 1; i <= _groupnum; i++)
                                    nochoice_row["G" + i.ToString()] = (decimal)nochoice_row["G" + i.ToString()] / (int)nochoice_row["frequency"] * 100;
                            ...
                            for (i = 1; i <= _groupnum; i++)
                            {
                                ...
                                foreach (DataRow temp_dr in temp.single_detail.Rows)
                                {
                                    if (!(temp_dr["mark"].ToString().Trim().Equals("未选或多选") ||
                                     ...
                                     temp_dr["mark"].ToString().Trim().Equals("得分率")))
                                        if (total != 0)
                                            dist_row[temp_dr["mark"].ToString().Trim()] = ((decimal)temp_dr["G" + i.ToString().Trim()] * (int)temp_dr["frequency"]/ 100)/total;
                                        else
                                            dist_row[temp_dr["mark"].ToString().Trim()] = 0;
                                }
                    ...
                    else
                    {
                        ...
                        foreach (DataRow row in temp.single_detail.Rows)
                        {
                            if ((int)row["frequency"] != 0)
                                for (i = 1; i <= _groupnum; i++)
                                    row["G" + i.ToString().Trim()] = (decimal)row["G" + i.ToString().Trim()] / (int)row["frequency"] * 100;
                        }
